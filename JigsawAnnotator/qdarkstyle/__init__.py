    ...
    try:
        ...
        if not is_pyqtgraph:
            if qt_api in QT_API_VALUES:
                ...
                raise KeyError(
                    "QT_API=%s is unknown, please use a value "
    ...
    try:
        ...
        if is_pyqtgraph:
            _logger().error(
                ...
                "and choose one option from %s", PYQTGRAPH_QT_LIB_VALUES)
    else:
        if is_pyqtgraph:
            if pyqtgraph_qt_lib in PYQTGRAPH_QT_LIB_VALUES:
                ...
                # Raise this error because the function need this key/value
                raise KeyError(
                    "PYQTGRAPH_QT_LIB=%s is unknown, please use a "
                    "value from %s",
    ...
    if qt_api and pyqtgraph_qt_lib:
        if qt_api != pyqtgraph_qt_lib.lower():
            _logger().warning(
                ...
                "some issues if using them in the same project!", qt_api,
                pyqtgraph_qt_lib)
