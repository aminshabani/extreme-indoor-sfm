...
class Network(object):
    ...
    @layer
    def fusion(self, inputs, split_w, name):

        with tf.variable_scope(name) as scope:
            input_groups = tf.split(inputs, split_w, 2)
            output = tf.concat(axis=3, values=input_groups)

        return output

    @layer
    def split_fusion(self, inputs, split_d, name):

        with tf.variable_scope(name) as scope:
            input_groups = tf.split(inputs, split_d, 3)
            output = tf.concat(axis=2, values=input_groups)

        return output

    @layer
    def feature_concat(self, inputs, split_w, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            fusion_num = len(input_groups)
            for i in range(0, fusion_num):
                tmp = []
                tmp.append(input_groups[i])
                for j in range(0, fusion_num):
                    if j != i:
                        tmp.append(input_groups[j])

                tmp = tf.concat(axis=3, values=tmp)
                print(tmp)
                fusion_feature.append(tmp)

            output = tf.concat(axis=2, values=fusion_feature)
            print(output)
    ...
    def batch_normalization(self,
        ...
        with tf.variable_scope(name) as scope:
            ...
            pop_mean = tf.get_variable(
                ...
                trainable=False)
            pop_var = tf.get_variable("variance",
                                      shape,
    ...
    def unpool_as_conv(self,
        ...
        if BN:
            ...
            self.feed(Y)
            self.batch_normalization(name=layerName,
                                     scale_offset=True,
                                     relu=False)
            Y = self.get_output()
    ...
    def up_project(self, size, id, stride=1, BN=True):
        ...
        branch2_output = self.unpool_as_conv(size,
                                             input_data,
                                             id_br2,
                                             stride,
                                             ReLU=False)
