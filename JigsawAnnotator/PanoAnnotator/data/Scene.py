...
class Scene(object):
    ...
    def loadOldLabel(self, path):
        path = path.replace('clean_data', 'preds_data/layout_preds')
        path = path.replace('.png', '.json')
        utils.loadOldLabelByJson(path, self)
    ...
    def __initDepth(self):
        ...
        panoDepthPath = os.path.join(self.__mainDirPath,
                                     pm.depthFileDefaultName)
        isExist = self.__panoDepth.initByImageFileDepth(panoDepthPath)
        if isExist:
            depthData = self.__panoDepth.data.astype(
                float) / 4000  #For Matterport3d GT
            self.__panoDepth.data = depthData
        else:
            if self.depthPred:
                # pred = self.depthPred.predict(self.__panoColor.image)
                pred = self.depthPred(self.__panoColor.path)
